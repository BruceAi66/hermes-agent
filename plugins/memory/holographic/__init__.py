...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def on_memory_write(self, action: str, target: str, content: str) -> None:
        ...
        if action == "add" and self._store and content:
            try:
                self._store.add_fact(content, category="user_pref" if target == "user" else "general", origin="owner")
            except Exception as e:
    ...
    _TOOL_HANDLERS = {
        "fact_store": _tool_handler({
            "add": lambda self, a: json.dumps({"fact_id": self._store.add_fact(
                a["content"], category=a.get("category", "general"), tags=a.get("tags", ""),
                origin=a.get("origin", "agent")), "status": "added"}),
        ...
        for msg in messages:
            ...
            for patterns, category in _EXTRACT_CATEGORIES:
                if any(p.search(content) for p in patterns):
                    try:
                        self._store.add_fact(content[:400], category=category, origin="owner")
                        extracted += 1
                    except Exception:
                        pass
        if extracted:
