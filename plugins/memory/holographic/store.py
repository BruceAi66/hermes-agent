...
class MemoryStore:
    ...
    def add_fact(self, content: str, category: str = "general", tags: str = "",
        ...
        ``origin`` is a provenance classification (owner/agent/untrusted/system)
        mirroring the memory-provenance design: content from the user directly is
        ``owner``, agent-derived from owner content is ``agent``, externally-sourced
        (web/tool output) is ``untrusted``, scaffolding (cron/heartbeat) is ``system``.
        Classification is conservative — callers that cannot determine provenance
        should pass ``agent`` rather than assume ``owner``.
        """
        with self._lock:
            ...
            try:
                fact_id: int = self._write("INSERT INTO facts (content, category, tags, trust_score, origin) VALUES (?, ?, ?, ?, ?)",
                                           (content, category, tags, self.default_trust, origin)).lastrowid  # type: ignore[assignment]
